...
use self::step::Step;
use self::step::StepEvent;
pub(super) use self::text::StrStep;
pub(super) use self::text::StrWithTokens;
...
pub(super) fn parse_type_list(input: &Input) -> Output {
...
/// Steps that can be used to construct the type-comment tree.
pub struct Output {
    steps: Vec<Step>,
}

/// A parser for Starlark type comments. It takes non-trivia tokens as input and processes that stream
